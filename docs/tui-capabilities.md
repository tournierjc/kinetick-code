...
| Updates | Update entry points and install-source detection restored; a fork installation updates from this fork's GitHub Releases (checksum-verified archive), an upstream installer installation keeps the upstream signed channel, and the upstream npm registry is reachable only through `KCODE_UPDATE_SOURCE=upstream` | Fork-release, application and service tests; live read-back of the published release; no real upgrade of an installed CLI on the development machine |
